...
for line in fr.readlines():
    if line:
        ...
        file_object = open(os.path.join(txt_path,filename[:-4] + ".txt"), 'w') #写文件
    ...
    for object in root.findall('object'): #找到root节点下的所有object节点 
        ...
        file_object.write(str(float(x1))+' '+str(float(y1))+' '+str(float(x2))+' ' +
                          str(float(y2))+' '+str(float(x4))+' '+str(float(y4))+' '+str(float(x3))+' '+str(float(y3))+' '+name +' 0')
        file_object.write('\n')
    file_object.close()